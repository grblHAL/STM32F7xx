...
void ioports_event (uint32_t bit)
{
    ...
    do {
        ...
        if((aux_in[port].bit & bit) && aux_in[port].interrupt_callback)
            aux_in[port].interrupt_callback(ioports_map_reverse(&digital.in, port), DIGITAL_IN(aux_in[port].port, aux_in[port].bit));
    } while(port);
...
static void on_settings_loaded (void)
{
    ...
    if(digital.out.n_ports) do {
        port--;
        DIGITAL_OUT(aux_out[port].port, aux_out[port].bit, (settings.ioport.invert_out.mask >> port) & 0x01);
    } while(port);
