...
#if ETHERNET_ENABLE && (_WIZCHIP_ == 5105 /*W5100S*/ || _WIZCHIP_ == 5500 /*W5500*/)
...
static void wizchip_select (void)
{
    ...
    DIGITAL_OUT(hw.cs.port, hw.cs.bit, 0);
}

static void wizchip_deselect (void)
{
    DIGITAL_OUT(hw.cs.port, hw.cs.bit, 1);
...
{
    if(hw.rst.port) {
        DIGITAL_OUT(hw.rst.port, hw.rst.bit, 0);
        hal.delay_ms(2, NULL);
        DIGITAL_OUT(hw.rst.port, hw.rst.bit, 1);
